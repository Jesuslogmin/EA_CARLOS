const url = "/v1/empleados"; // URL actualizada para empleados
...
function save(bandera) {
    ...
    ajaxRequest(type, endpoint, empleado)
        .done((data) => {
            if (data.ok) {
                ...
                Swal.fire({
                    icon: 'success',
                    title: `Se ha ${bandera === 1 ? 'guardado' : 'actualizado'} el empleado`,
        ...
        .done((data) => {
            ...
            if (data.ok) {
                $.each(data.body, (index, empleado) => {
                    ...
                    t.row.add([botonera, empleado.id, empleado.nombre, empleado.direccion, empleado.telefono, empleado.email, empleado.createdAt]);
                });
                t.draw(); 
            } else {
...
$(document).ready(function () {
    $("#tablaRegistros").DataTable({
        language: {
            ...
            info: "Mostrando del _START_ al _END_ de _TOTAL_ empleados",
    ...
    $(document).on('click', '.eliminar', function () {
        ...
        Swal.fire({
            ...
            text: "¿Está seguro de querer eliminar este empleado?",
            icon: 'warning',
